...
def get_thresholds_from_feature(feature, bins):
    ...
    if len(peaks_idx) == 0:
        ...
        M1 = (
            0.5
            * (
                np.concatenate(([0], edges_feature))
                + np.concatenate((edges_feature, [0]))
            )[peaks_idx[0] + 1]
        )
        M2 = min(feature)
    else:
...
def detect_speech(
    audio_in, fs, win_length=None, hop_length=None, merge_distance=None, thresholds=None
):
    """Parameters:
    ...
    spectral_spread_threshold = (
        0.05  # Threshold used for not counting spectral spread when under this energy
    )
    ...
    frames = librosa.util.frame(
        audio_in, frame_length=frame_length, hop_length=hop_length
    )
    ...
    f_energy = median_filter(
        median_filter(energy, size=smoothing_filter_length),
        size=smoothing_filter_length,
    )
    ...
    f_spec_spread = median_filter(
        median_filter(spec_spread, size=smoothing_filter_length),
        size=smoothing_filter_length,
    )
    ...
    # plt.plot(f_energy)
    speech_mask = (f_spec_spread > sspread_thresh / 30) & (
        f_energy > energy_thresh / 10
    )
